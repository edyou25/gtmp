...
W
| 算法名称                      | 核心优势                                                 | 核心劣势                                                 | 适用场景/特点                                                    |
| :---------------------------- | :------------------------------------------------------- | :------------------------------------------------------- | :--------------------------------------------------------------- |
| **GTMP**                      | - **极高批量规划效率** (比基线快10000-100000倍)         | - **路径平滑度较差** (Min Cosim最差)，路径可能粗糙    | - 大规模数据收集、机器人学习等需要大量**多样化**路径的批量规划 |
|                               | - GPU/TPU高效**向量化** (张量操作)                       | - 内存需求较高 (尤其GPU加速时)                         | - 需要可预测、固定段数的路径生成                           |
|                               | - 最佳**路径多样性** (PD)                                | - 单次规划时路径质量（如长度）可能不如特定基线           | - 无需路径简化和梯度计算                                     |
|                               | - 规划时间分布可预测且集中                               |                                                          |                                                                  |
| **GTMP-Akima**                | - 继承GTMP的**批量效率优势**                             | - 规划时间略高于纯GTMP                                   | - 需要批量高效生成**相对平滑**路径的场景                   |
|                               | - **无需梯度**即可规划**平滑样条路径** (C1连续)          | - 平滑度为C1连续，更高阶连续性待探索                   | - 可作为其他需要平滑初始轨迹的优化器（如GPMP）的预热方法 |
|                               | - 平滑度（Min Cosim）可与cuRobo媲美                     | - 概率完备性理论尚待证明                                 |                                                                  |
| **传统采样基规划器**          | - **概率完备性** (能够找到可行路径)                      | - **批量规划效率极低** (比GTMP慢100000倍)              | - 单次、非实时、对计算效率要求不那么高的规划任务         |
| **(RRTConnect, BKPIECE, OMPL)** | - 算法成熟，研究广泛                                     | - 不利于**实例级向量化**                                 | - 适用于需要理论完备性的基础研究                         |
|                               |                                                          | - 需要额外的**路径简化例程**进行后处理                   |                                                                  |
| **VAMP/RRTC**                 | - **高效的碰撞检测** (微秒级)                            | - **实例级向量化受限** (批量效率仍远低于GTMP)          | - 对单个规划实例的碰撞检测效率要求极高的场景             |
|                               | - 利用CPU-based SIMD实现**部分向量化** (碰撞检测)      | - 仍需**路径简化和B样条平滑**进行优化                    |                                                                  |
| **基于梯度的轨迹优化器**      | - 生成**高质量、高度平滑的轨迹**                         | - **批量规划效率低** (比GTMP慢2500倍)                  | - 对路径**平滑度、动力学约束和局部优化质量**要求极高的任务 |
| **(CHOMP, GPMP, cuRobo)**     | - 可考虑**动力学约束** (如cuRobo)                         | - 对**初始条件敏感**                                     | - 适用于需要精细轨迹优化（例如，机器人操纵、安全路径）的场景 |
|                               | - 能够找到**多模态解决方案**                             | - 可能陷入**不可行区域**                                 |                                                                  |
|                               | - 对局部最小值具有一定鲁棒性                             | - 依赖于**规划成本的梯度信息**                           |                                                                  |
